...
{
	_currentFileIndex = -1;
	// use absolute path,otherwise having problem when use it with ros
	string abs_svg_path = _SETTINGS->stage.svg_path[0] != '/' ? GLOBAL_PACKAGE_PATH +"/"+_SETTINGS->stage.svg_path : _SETTINGS->stage.svg_path;

	const char *folder_path = abs_svg_path.c_str();
