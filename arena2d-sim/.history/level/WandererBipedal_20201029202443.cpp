...
WandererBipedal::WandererBipedal(b2World * w, const b2Vec2 & position, float velocity, unsigned int type, unsigned int mode=0,
	                std::vector<b2Vec2> waypoints={}, int stop_counter_threshold=1, 
                    float change_rate=1.0f, float stop_rate=0.1f, float max_angle_velo = 60.0f):Wanderer(w, position, velocity,type,mode,waypoints,stop_counter_threshold, change_rate, stop_rate, max_angle_velo)
{
