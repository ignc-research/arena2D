...
class Arena2dEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # automaticlly lock and release the mutex
        with self.response_con:
            self.response_con.wait(10)
            if not self.resp_received:
                rospy.logerr("Env didn't get the feedback from arena simulator")
            self.resp_received = False
        return self.obs, self.reward, self.done, self.info

    def reset(self):
        self._pubRosAgentReq(None, True)
        # same as step function
        with self.response_con:
            self.response_con.wait(10)
            if not self.resp_received:
                rospy.ERROR("Env didn't get the feedback from arena simulator")
        return self.obs, self.reward, self.done, self.info
    ...
    def _setSubPub(self):
        ...
        rospy.loginfo("env[{:d}] waiting for arena-2d simulator to connect!".format(self._idx_env))
        times = 0
        # subscriber
        ...
        while self._ros_agent_pub.get_num_connections() == 0 or self._arena2d_sub.get_num_connections() == 0:
            time.sleep(0.1)
            times +=1
        rospy.loginfo("env[{:d}] arena-2d simulator connected! took {:3.1f}s.".format(self._idx_env,.1 * times))
        # time.sleep(1)

    def _pubRosAgentReq(self, action: Union[List, Tuple, RosAgentReq] = None, env_reset: bool = True):
        req_msg = RosAgentReq()
        # reset environment
        if env_reset:
